...
def query_criar_database(dataframe, nome_database, lista_colunas):
    ...
    for coluna in lista_colunas:
        if coluna == 'DESCRICAO':
            create_query_criar_colunas += f'`{coluna}` varchar(80) NULL,'
        elif coluna == 'FORNECEDOR':
            create_query_criar_colunas += f'`{coluna}` varchar(70) NULL,'
        else:
...
nome_table = SQL_dados.nome_table
...
def read_and_change_data_frame(path, header, file_type):
    if file_type == 'xlsx':
        dataframe = pd.read_excel(path, header=header)
    elif file_type == 'csv':
        dataframe = pd.read_csv(path, header=header)
    else:
        print('Especify a valid type, or change "read_and_change_data_frame" function')
...
if __name__ == '__main__':
    data = read_and_change_data_frame(path='Pasta1.xlsx', header=9, file_type='xlsx')
    ...
    #
    # # Exportar dados
    # BD_to_SQL(pd.read_csv('export.csv'))
    # =====================================================================================
